...
async function getAllUserStats() {
    try {
        const { rows: users } = await client.query(`
            SELECT username, betscorrect, totalbets, lockscorrect, totallocks, totalpoints, parlayscorrect, totalparlays, wins, currentwinner, totalcorrectpickem, totalpickem, pickemwins, currentpickemwinner
            FROM users
            ORDER BY id;
        `)
